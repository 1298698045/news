	...
	<view class="wrapper">
		<div class="listWrap">
			<div class="itemBox" v-for="(item,index) in listData" :key="index">
				...
				<div class="options" @click="[0,1].includes(item.StatusCode.statusId) && gotoExam(item)">
					<button class="btn">{{item.StatusCode.statusId == 0 ? '开始答题' : item.StatusCode.statusId == 1 ? '继续答题' : '已完成'}}</button>
				</div>
			</div>
		</div>
	</view>
